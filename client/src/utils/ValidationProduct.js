export const validateProduct = (product, validCategories = []) => {
  ...
  if (product.price === null || product.price === undefined || String(product.price).trim() === "") {
  ...
  if (product.stock === null || product.stock === undefined || String(product.stock).trim() === "") {
  ...
  } else if (validCategories.length > 0 && !validCategories.includes(category)) {
